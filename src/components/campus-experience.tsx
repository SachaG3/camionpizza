...
export function CampusExperience({ user, onOrder, onAccount, surface = "quiz" }: { surface?: "quiz" | "battle" | "club"; user: PublicLoyaltyUser | null; onOrder: (pizza: Pizza) => void; onAccount: () => void }) {
  ...
  return <>
    ...
    <CampusFrame surface={surface} mode={mode} onClose={() => setMode(null)}>
        ...
        {mode === "club" && <div className="campus-club">
          <section className="club-pass" aria-label={`${user?.stamps ?? 0} tampons sur 6`}>
            <div className="club-pass-brand"><span>FOURCHETTE SOCIAL CLUB</span><Ticket size={22} /></div>
            <div className="club-balance"><div><strong>{user?.stamps ?? 0}<small>/6</small></strong><span>tampons</span></div><p>{user?.rewards ? <><b>{user.rewards} pizza{user.rewards > 1 ? "s" : ""} offerte{user.rewards > 1 ? "s" : ""}</b><br />disponible{user.rewards > 1 ? "s" : ""}</> : <><b>Encore {6 - (user?.stamps ?? 0)}</b><br />avant ta pizza offerte</>}</p></div>
            <div className="club-stamps">{Array.from({ length: 6 }, (_, i) => <span key={i} className={i < (user?.stamps ?? 0) ? "earned" : ""}>{i < (user?.stamps ?? 0) ? <Check /> : String(i + 1).padStart(2, "0")}</span>)}</div>
            <div className="club-pass-footer"><span>{user?.name ?? "Membre Fourchette"}</span><strong>MEMBER’S EDITION</strong></div>
          </section>
          <section className="club-missions">
            <div className="club-missions-head"><span className="campus-kicker">DÉFIS DU CAMPUS</span><strong>{Number(data?.club.quiz) + Number(data?.club.vote)}/2</strong></div>
            <button onClick={() => { onAccount(); window.dispatchEvent(new Event("fourchette-open-quiz")); }}><span className={`mission-check ${data?.club.quiz ? "done" : ""}`}>{data?.club.quiz ? <Check /> : "01"}</span><span><strong>Trouve ton alter ego gourmand</strong><small>{data?.club.quiz ? "Pizza match terminé" : "Termine le pizza match"}</small></span><ArrowRight /></button>
            <button onClick={() => { onAccount(); document.querySelector("#campus-battle")?.scrollIntoView({ behavior: "smooth" }); }}><span className={`mission-check ${data?.club.vote ? "done" : ""}`}>{data?.club.vote ? <Check /> : "02"}</span><span><strong>Fais entendre ton campus</strong><small>{data?.club.vote ? "Vote enregistré" : "Vote dans la battle"}</small></span><ArrowRight /></button>
            <div className={`club-reward ${data?.club.rewardCode ? "unlocked" : ""}`}><Trophy /><div><h3>{data?.club.rewardCode ? "Badge Pionnier débloqué" : "Badge Pionnier"}</h3><p>{data?.club.rewardCode ? "Tu fais partie des premiers membres du Club." : "Termine les deux défis pour rejoindre les pionniers."}</p></div>{data?.club.rewardCode ? <strong className="reward-code">{data.club.rewardCode}</strong> : <button disabled={busy || !data?.club.quiz || !data?.club.vote} onClick={() => void act({ action: "claim" })}>Débloquer</button>}</div>
          </section>
          <a className="club-order-cta" href="#menu" onClick={onAccount}>Commander pour gagner un tampon <ArrowRight size={17} /></a>
          <p className="club-terms">1 tampon par pizza · la 6e débloque une pizza offerte.</p>
        </div>}
